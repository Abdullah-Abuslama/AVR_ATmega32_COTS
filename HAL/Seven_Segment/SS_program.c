...
{
	DIO_SetPort_Direction(A_PortNum,PORT_OUTPUT);
}
...
void HSSD_VoidDisplayNumber(u8 A_u8SSD_PORT ,u8 A_u8Num)
{
	if(A_u8Num <=9){
		#if SSD_Type==1
			DIO_SetPort_value(A_u8SSD_PORT,   G_u8Ssd_NUMs[A_u8Num]);
		#elif SSD_Type==2
			DIO_SetPort_value(A_u8SSD_PORT, ~ G_u8Ssd_NUMs[A_u8Num]);
		#endif
	}
...
{
	#if SSD_Type==1
		DIO_SetPort_value(A_u8SSD_PORT, 0x00);
	#elif SSD_Type==2
		DIO_SetPort_value(A_u8SSD_PORT, 0xff);
	#endif
}
